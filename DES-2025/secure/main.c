...
typedef int __attribute__((cmse_nonsecure_call)) ns_callback_func(int result);
typedef int (*ns_funcptr)(int) __attribute__((cmse_nonsecure_call));

void __attribute__((cmse_nonsecure_entry)) sec_sum_callback(int a, int b, ns_funcptr cb_ns)
{
	int sum = a+b;
	logPrint("S: Secure world got (%x,%x), sum=%x\n",a,b,sum);
	
	ns_funcptr cb = (ns_funcptr)cmse_nsfptr_create(cb_ns);
	int result = cb(sum);
	logPrint("S: Returned from NS callback, result=%x\n",result);
}

/* sec_sum, s_call_ns functions

void __attribute__((cmse_nonsecure_entry)) sec_sum(int a, int b)
{
	logPrint("Secure : received result=%x\n", a+b);
}

void __attribute__((cmse_nonsecure_entry)) s_call_ns(ns_funcptr cb)
{
	logPrint("Secure : entered s_call_ns()\n");
	ns_funcptr ns_cb = (ns_funcptr)cmse_nsfptr_create(cb);

	logPrint("Secure : calling into NS...\n");
	ns_cb();

	logPrint("Secure : back in Secure\n");
}
